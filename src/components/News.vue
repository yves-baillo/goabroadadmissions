        ...
        <div class="container mx-auto px-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div 
              ...
              class="bg-gray-900 rounded-xl overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 group border border-gray-800 hover:border-yellow-500/50"
            >
              <div class="relative h-48 overflow-hidden" @click="openLink(item.link)">
                <img 
                  :src="item.image" 
              ...
              <div class="p-6">
                <h3 class="text-xl font-bold text-white mb-2 line-clamp-2 group-hover:text-yellow-400 transition-colors cursor-pointer" @click="openLink(item.link)">
                  {{ item.title }}
                </h3>
                <p class="text-gray-400 mb-4 line-clamp-3 cursor-pointer" @click="openLink(item.link)">{{ item.message }}</p>
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <!-- Request Assistance Button - Links to Google Form -->
                    <button 
                      @click.stop="openAssistanceForm(item)"
                      class="bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-lg text-sm flex items-center gap-1 transition-all duration-300 transform hover:scale-105"
                      title="Request help with this scholarship"
                    >
                      <HelpCircle :size="14" />
                      Request Assistance
                    </button>
...
import { Bell, Bookmark, Eye, Clock, Award, ArrowRight, HelpCircle } from 'lucide-vue-next'
...
// Google Form URL
const GOOGLE_FORM_URL = 'https://docs.google.com/forms/d/e/1FAIpQLSdel6b8-7EZ3nr1OleLs9bEony-WgymoLs1l0Dag0FyFxtSPQ/viewform'
...
// Open Google Form for assistance request
const openAssistanceForm = (item) => {
  // You can optionally pre-fill the form with scholarship info
  // Note: You'll need to get the actual form field IDs from the Google Form
  // For now, just open the form
  window.open(GOOGLE_FORM_URL, '_blank')
  
  // Optional: Log which scholarship user needs help with
  console.log('User requested assistance for:', {
    title: item.title,
    source: item.source,
    link: item.link
  })
